...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        relu1 = relu_forward(affine1[0])
        affine2 = affine_forward(relu1[0], self.params['W2'], self.params['b2'])
        scores = affine2[0]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        temp = 0
        temp += (self.params['W1']**2).sum()
        temp += (self.params['W2']**2).sum()
        loss += self.reg*0.5*temp
        
        affine2_back = affine_backward(dx, affine2[1])
        grads['W2'] = affine2_back[1]+self.reg*affine2[1][1]
        grads['b2'] = affine2_back[2]
    
        relu1_back = relu_backward(affine2_back[0], relu1[1])
                
        affine1_back = affine_backward(relu1_back, affine1[1])
        grads['W1'] = affine1_back[1]+self.reg*affine1[1][1]
        grads['b1'] = affine1_back[2]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if self.use_batchnorm:
            self.params['W'+str(1)] = np.random.normal(0, weight_scale, [input_dim, hidden_dims[0]])
            self.params['b'+str(1)] = np.zeros([hidden_dims[0], ])
            self.params['gamma'+str(1)] = np.ones([hidden_dims[0], ])
            self.params['beta'+str(1)] = np.ones([hidden_dims[0], ])
    ...
    def loss(self, X, y=None):
        ...
        affine_cache = affine_caches.pop()
        dx, dw, db = affine_backward(dx, affine_cache)
        grads['W'+str(self.num_layers)] = dw+self.reg*affine_cache[1]
        grads['b'+str(self.num_layers)] = db
        
        for i in range(1, self.num_layers).__reversed__():
            if self.use_dropout:
                dropout_cache = dropout_caches.pop()
                dx = dropout_backward(dx, dropout_cache) 
            
            relu_cache = relu_caches.pop()
            dx = relu_backward(dx, relu_cache)
            
            if self.use_batchnorm:
                batch_cache = batch_caches.pop()
            ...
            affine_cache = affine_caches.pop()
            dx, dw, db = affine_backward(dx, affine_cache)
            grads['W'+str(i)] = dw+self.reg*affine_cache[1]
            grads['b'+str(i)] = db
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
